...
class Preprocesser:
    ...
    def _upsample_nwp_data(self, df):
        # 对每个lat和lon的数据进行上采样，从1小时扩展到15分钟
        df['time'] = pd.to_datetime(df['time'])
        
        # 创建15分钟间隔的时间索引
        time_range = pd.date_range(
            start=df['time'].min(),
            end=df['time'].max() + pd.Timedelta(minutes=45),
            freq='15min'
        )
        ...
        for (lat, lon), group in df.groupby(['lat', 'lon']):
            # 创建完整的时间索引
            full_time_index = pd.DataFrame({'time': time_range})
            ...
            # 线性插值
            merged = merged.interpolate(method='linear', limit=4)
            
            # 填充lat和lon列
            merged['lat'] = lat
            merged['lon'] = lon
            
            upsampled_dfs.append(merged)
        
        # 合并所有上采样后的数据
        df = pd.concat(upsampled_dfs, ignore_index=True)
        return df
        ...
        try:
            ...
            if dataset == 'train':
                ...
                time_index = pd.date_range(start='2025-01-01 00:00:00',
                                           ...
                                           freq='15min')
                merged_df = pd.DataFrame({'time': time_index, 'power': None})
...
if __name__ == "__main__":
    ...
    preprocesser = Preprocesser(data_dir=data_dir)

    # # 处理并保存所有气象数据和功率数据
    # print("开始处理所有气象数据...")
    # # 处理训练集
    # preprocesser.process_and_save_all_nwp_data(dataset='train')
    # # 处理测试集
    # preprocesser.process_and_save_all_nwp_data(dataset='test')

    # print("\n开始处理所有功率数据...")
    # preprocesser.process_and_save_all_power_data()
    ...
    #         # 测试加载原始数据
    #         print("从原始目录加载:")
    #         df = preprocesser.load_raw_nwp_data(station_id, source)
    #         if df is not None:
    ...
    #         # 测试加载处理后的数据
    #         print("\n从处理后目录加载:")
    #         df = preprocesser.load_processed_nwp_data(station_id, source)
    #         if df is not None:
    #             print(f"成功加载数据，时间范围: {df['time'].min()} 到 {df['time'].max()}")
    #             print(f"数据列: {df.columns.tolist()}")
    #             print(f"数据形状: {df.shape}")
    ...
    #     # 测试加载原始数据
    #     print("从原始目录加载:")
    #     power_df = preprocesser.load_raw_power_data(station_id)
    ...
    #     # 测试加载处理后的数据
    #     print("\n从处理后目录加载:")
    #     power_df = preprocesser.load_processed_power_data(station_id)
    #     if power_df is not None:
    #         print(
    #             f"成功加载数据，时间范围: {power_df['time'].min()} 到 {power_df['time'].max()}"
    #         )
    ...
    #         # 首次合并数据（会创建缓存）
    #         print("首次合并数据...")
    #         merged_data = preprocesser.merge_power_and_nwp_data(station_id, dataset)
    #         if merged_data is not None:
    #             print(f"合并数据形状: {merged_data.shape}")

    # # 从缓存加载数据
    # print("\n从缓存加载数据...")
    # cached_data = preprocesser.merge_power_and_nwp_data(station_id, dataset)
    # if cached_data is not None:
    #     print(f"缓存数据形状: {cached_data.shape}")

    # # 强制重新合并数据
    # print("\n强制重新合并数据...")
    # remerged_data = preprocesser.merge_power_and_nwp_data(station_id,
    #                                                 dataset,
    #                                                 force_remerge=True)
    # if remerged_data is not None:
    #     print(f"重新合并数据形状: {remerged_data.shape}")

    # train1 = preprocesser.merge_power_and_nwp_data(1, 'train')
    # test1 = preprocesser.merge_power_and_nwp_data(1, 'test')
