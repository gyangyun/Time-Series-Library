...
from utils.losses import CustomLoss, asymmetric_mse_loss, RenewableEnergyLoss
...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def _select_criterion(self):
        if hasattr(self.args, 'loss') and self.args.loss == 'renewable':
            criterion = RenewableEnergyLoss(
                alpha=self.args.loss_alpha
                if hasattr(self.args, 'loss_alpha') else 1.5,
                beta=self.args.loss_beta
                if hasattr(self.args, 'loss_beta') else 0.8,
                gamma=self.args.loss_gamma
                if hasattr(self.args, 'loss_gamma') else 0.3,
                delta=self.args.loss_delta
                if hasattr(self.args, 'loss_delta') else 0.2)
        else:
    ...
    def vali(self, vali_data, vali_loader, criterion):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark,
                ...
                # 把未来协变量添加到dec_inp中
                if self.args.use_future_covariates:
                    future_cov_dims = [
                        int(dim)
                        for dim in self.args.future_cov_dims.split(",")
                    ] if self.args.future_cov_dims else list(
                        range(batch_y.shape[-1]))
                    dec_inp[:, :,
                            future_cov_dims] = batch_y[:, -self.args.pred_len:,
                                                       future_cov_dims]
